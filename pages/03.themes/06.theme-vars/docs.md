...
When you are designing a theme, Kunena gives you access to all sorts of objects and variables from your twig templates.  The Twig templating framework provides powerful ways to read and manipulate these objects and variables.  This is [fully explained in their own documentation](http://twig.sensiolabs.org/doc/templates.html) as well as [summarized succinctly in our own documentation](../twig-primer).
...
The `{{ base_dir }}` variable returns the base file directory of the Kunena installation.

### base_url variable

The `{{ base_url }}` returns the base URL to the Kunena site, whether or not this shows the full URL is dependent on the `absolute_urls` [option in the system configuration](../../basics/Kunena-configuration#system-configuration).

### base_url_relative variable

The `{{ base_url_relative }}` returns the base URL to the Kunena site, without the host information.

### base_url_absolute variable

The `{{ base_url_absolute }}` returns the base URL to the Kunena site, including the host information.
...
You can access any Kunena configuration setting via the config object as set in the `configuration.yaml` file.  For example:
...
Because Kunena is built using the structure defined in the `pages/` folder, each page is represented by a **page object**.
...
This returns whether or not a page is routable by Kunena.  This means if you can point your browser to the page and receive content back.  Non-routable pages can be used in templates, plugins, etc, but cannot be reached directly. This can be set in the page headers:
...
This returns the full file path of the page. For example `/Users/yourname/sites/Kunena/user/pages/01.home/default.md`

##### filePathClean()

This returns the relative path from the root of the Kunena site.  For example `user/pages/01.home/default.md`

##### path()

This returns the full path to the directory containing the page.  For example `/Users/yourname/sites/Kunena/user/pages/01.home`
...
The **pages object** represents a nested tree of every **page object** that Kunena knows about.  This is particularly useful for creating a **sitemap**, **navigation** or if you wish to find a particular **page**.

##### children method

This returns the immediate child pages as an array of **page objects**. As the pages object represents the entire tree, you can fully recurse over every page in the Kunena pages/ folder.
...
The Uri object has several methods to access parts of the current URI. For the full URL `http://mysite.com/Kunena/section/category/page.json/param:foo?query=bar`:
...
This returns the route as either an absolute or relative URL.  (e.g. `uri.route(true)` = `http://mysite.com/Kunena/section/category/page` or `uri.route()` = `/section/category/page`)
...
This returns the full URL with or without the host.  (e.g. `uri.url(false)` = `Kunena/section/category/page/param:foo?query=bar`)
...
This returns the root url to the Kunena instance.  (e.g. `uri.rootUrl()` = `http://mysite.com/Kunena`)
...
Kunena has built-in support for programmatically determining the platform, browser, and version of the user.
