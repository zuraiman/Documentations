...
With **Kunena** the ability to group or tag pages is baked right into the system with **Taxonomy**.
...
1. Define a list of Taxonomy types in your [`site.yaml`](../../basics/Kunena-configuration)
...
Accomplishing this in Kunena is a simple procedure.  Kunena provides a default `site.yaml` file that is located in the `system/config` folder.  By default that configuration defines two taxonomy types of `category` and `tag`:
...
This will override the taxonomies that Kunena knows about so that pages can be assigned to any of these three taxonomies.
