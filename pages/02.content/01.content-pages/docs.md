...
In Kunena-speak, **pages** are the fundamental building blocks of your site.  They are how you write content and provide navigation in the Kunena system.
...
Kunena natively supports **3 types of pages** that allow you to create a rich selection of web content. Those types are:

{{ media['page-types.png'].html('Kunena Page Types') }}

#### Standard Page

{{ media['content-standard.jpg'].html('Standard Page','border') }}

A standard page is generally a single page such as a **blog post**, **contact form**, **error page** etc. This is the most common type of page that you will create. By default a page is considered a standard page unless you tell Kunena otherwise.

When you download and install the **Base Kunena** package, you are greeted by a standard page.  We covered creating a simple standard page in the [Basic Tutorial](../../basics/basic-tutorial).
...
>>> A sample **Blog Skeleton** using a **Listing Page** can be found in the [Kunena Downloads](http://getKunena.org/downloads).
...
>>> A sample **One-Page Skeleton** using a **Modular Page** can be found in the [Kunena Downloads](http://getKunena.org/downloads).

Each of these page types follow the same basic structure, so before we can get into the nitty-gritty of each type, we must explain how pages in Kunena are constructed.
...
Kunena understands that any integer value followed by a period will be solely for the purpose of ordering, and is removed internally in the system.  For example, if you have a folder named `01.home`, Kunena will treat this folder as `home`, but will ensure that with default ordering, it comes before `02.blog`.

{{ media['page-folders.png'].html('Kunena Folder Example') }}

Your site must have an entry-point so that it knows where to go when you point your browser to the root of your site. For example if you were to enter `http://yoursite.com` in your browser,  by default, Kunena expects an alias `home/`, but you can override the home location by changing the `home.alias` option in the [Kunena configuration file](../../basics/Kunena-configuration).
...
>>> You can override the **default behavior** for folder ordering and the direction in which the ordering occurs by setting the `pages.order.dir` and the `pages.order.by` options in the [Kunena system configuration file](../../basics/Kunena-configuration).
...
You can of course name your file whatever you like, for example: `document.md`, which would cause Kunena to look for a template file in the theme that matches, such as the **document** template.
...
Kunena has a useful feature that allows you to find another page and perform actions on that page. This can be accomplished with the `find()` method that simply takes the **route** and returns a new page object.

This allows you to perform a wide variety of functionality from any page on your Kunena site.  For example, you may want to provide a list of all current projects on a particular project detail page:
