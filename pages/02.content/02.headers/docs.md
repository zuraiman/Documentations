...
The headers at the top of a page are completely optional, you do not need them at all for a page to display within Kunena. There are 3 primary types of pages (**Standard**, **Listing**, and **Modular**) within Kunena, and each has relevant headers.
...
If the `title` variable is not set, Kunena has a fall back solution, and will try to use the capitalized `slug` variable:
...
The `slug` variable allows you to specifically set the page's portion of the URL. For example: `http://yoursite.com/my-page-slug` would be the URL if you set the `slug` above.  If the `slug` is not set in the page, Kunena falls back to using the folder name (without any numerical prefixes).

[Slugs](http://en.wikipedia.org/wiki/Semantic_URL#Slug) are generally entirely lowercase, with accented characters replaced by letters from the English alphabet and whitespace characters replaced by a dash or an underscore. While future versions of Kunena will support spaces in slugs, having blank spaces or capital lettering is not recommended.
...
The `menu` variable lets you set the text to be used in the navigation. There are several layers of fall-backs for the menu, so if no `menu` variable is set, Kunena will try to use the `title` variable.
...
Kunena automatically looks for a page with the route `/error` if another page cannot be found.  With this being an actual page within Kunena, you would have complete control over what this page looks like.  You probably do not want people accessing this page directly in their browser, however, so this page commonly has its `routable` variable set to false. Valid values are `true` or `false`.
...
taxonomy:
    category: blog
    tag: [sample, demo, Kunena]
```

A very useful header variable, `taxonomy` lets you assign values to **taxonomy** you defined as valid types in the [Site Configuration](../../basics/Kunena-configuration#site-configuration) file.

If the taxonomy is not defined in that file, it will be ignored.  In this example, the page is defined as being in the `blog` category, and has the tags: `sample`, `demo`, and `Kunena`.  These taxonomies can be used to find these pages from other pages, plugins and even themes. The [Taxonomy](../taxonomy) chapter will cover this concept in more detail.
...
By default, Kunena will cache the contents of the page file to ensure things run as fast as possible.  There are advanced scenarios where you do not want the page to be cached.
...
Facebook mostly uses OpenGraph metatags, but there are some Facebook-specific tags and these are support automatically by Kunena.
...
To tell Kunena that a specific page should be a listing page and contain child-pages, there are a number of variables that can be used:
...
The `content.items` value tells Kunena to gather up a collection of items and specifically `@self.children` indicates that the collection should consist of the child pages below this page in the folder structure.
...
The `@self.modular` configuration option tells Kunena that the page should consist of all the modular pages that exist as children of this particular page.
...
Using the `@taxonomy` option, you can utilize Kunena's powerful taxonomy functionality.  This is where the `taxonomy` variable in the [Site Configuration](../../basics/Kunena-configuration#site-configuration) file comes into play. There **must** be a definition for the taxonomy defined in that configuration file for Kunena to interpret a page reference to it as valid.

By setting `@taxonomy.tag: foo`, Kunena will find all the pages in the `/user/pages` folder that have themselves set `tag: foo` in their taxonomy variable.
...
New as of **Kunena 0.9.13** is the ability to filter by a date range:
...
The significance of these headers is that Kunena does not use them by default, they are only read by the **sitemap plugin** to determine how often this particular page is modified and what its priority should be.
